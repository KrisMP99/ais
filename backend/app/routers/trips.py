...
async def get_trip(p1: Coordinate, p2: Coordinate): 
    ...
    WHERE ST_Intersects(std.line_string, ST_SetSRID(gp1.geom, 3857)) \
    AND ST_Intersects(std.line_string, ST_SetSRID(gp2.geom, 3857))) \
    \
    SELECT CASE \
        WHEN ST_Contains(pil.geom, ST_SetSRID(gp1.geom, 3857)) \
            THEN true \
        WHEN ST_Contains(pil.geom, ST_SetSRID(gp2.geom, 3857)) \
            THEN true \
        ELSE ST_Contains(pil.geom, ST_SetSRID(gp1.geom, 3857)) \
    END first_point\
